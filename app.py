...
''')
# Inserting Image
st.image('C:\\Users\\anjal\OneDrive\Desktop\imagescovid.png', caption="Image by Me",width=850)
# Reading csv data
data = pd.read_csv("C:\\Users\\anjal\\OneDrive\\Desktop\\college project1\Covid-19.csv")
...
graphs = st.sidebar.selectbox("Graphs & Plots", ("Bar Graph", "Scatter Plot", "HeatMap", "Pie Chart","Area chart","State-map"))
# Sorting the columns
...
def visualize_plotly(graph):
    if graph == "Bar Graph":
        st.write(graph)
        fig = px.bar(data, x=x_label, y=y_label, color=col,width=1000,height=700)

    elif graph == "Scatter Plot":
        st.write(graph)
        fig = px.scatter(data, x=x_label, y=y_label, color=col ,width=900,height=700)

    elif graph == "HeatMap":
        st.write(graph)
        fig = px.density_heatmap(data, x=x_label, y=y_label, nbinsx=30, nbinsy=30)

    elif graph=="Area chart":
        st.write(graph)
        fig=px.area(data,x=x_label,y=y_label ,color=col,width=900,height=700)
    elif graph=="State-map":
        st.write(graph)
        fig=px.scatter_geo(data,lat=x_label,lon=y_label,color=col, width=900,height=700)

    else:
        st.write(graph)
        fig = px.pie(data, values=x_label, names=data[y_label],width=900)
